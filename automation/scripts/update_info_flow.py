...
def write_update_notes(
    ...
    digest_lines = [
        ...
        "---",
        "",
        f"# {today} AI Embodied Intelligence Update",
        "",
        "## 运行摘要",
        "",
        f"- 候选数量：{candidate_count}",
        f"- 入选条目：{len(selected)}",
        f"- 其中回填已见条目：{repeated_count}",
    ...
    for item in selected:
        ...
        concept_line = " ".join(f"[[{concept}]]" for concept in concepts)
        link_path = f"30_Updates/{today}/{safe_title}"
        digest_lines.extend(
            [
                f"### [[{link_path}|{item['title']}]]",
                "",
                f"- Score: {score}",
                f"- Repeat fallback: {'yes' if item.get('repeat_fallback') else 'no'}",
                f"- Source: {item.get('source', '')}",
                f"- Published: {item.get('published', '')}",
                f"- Age days: {item.get('age_days', 'unknown')}",
...
def update_index(vault: Path) -> None:
    ...
    lines.extend(
        [
            ...
            "- 先看“为什么重要”，再看原文。",
            "- 每条更新至少挂回一个核心概念。",
            "- 如果一条信息不能改变知识网中的任何节点，先放进 `00_Inbox`。",
            "",
        ]
    )
