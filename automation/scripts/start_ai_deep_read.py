...
def create_deep_read(vault: Path, note: str, level: str) -> tuple[Path, bool]:
    ...
    source_link = source.relative_to(vault).with_suffix("").as_posix()
    output_dir = vault / "50_Papers" / "Deep Reads"
    output = output_dir / f"{flow.slugify(title, 120)} - 精读.md"
    index_path = vault / INDEX
    if output.is_file():
        register(index_path, output, source, vault, title, level)
        return output, False
    ...
    values = {
        ...
        "url_yaml": flow.yaml_string(str(metadata.get("url", ""))),
        "title": title,
